import { Tabs, TabsContent, TabsList, TabsTrigger } from 'xyzdocs-ui/components/ui/tabs';
import { CodeBlock } from '@/components/code-block';

export function Installation({ name }: { name: string }) {
  const tabs = [{ name: 'xyzdocs CLI', value: 'xyzdocs-cli' }];
  ...
  return (
    <Tabs className="my-6">
      ...
      <TabsContent value="xyzdocs-cli">
        <CodeBlock code={`npx @xyzdocs/cli@latest add ${name}`} lang="bash" />
      </TabsContent>
    </Tabs>
  );
}
