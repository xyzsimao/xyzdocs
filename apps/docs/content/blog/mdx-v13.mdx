...
xyzdocs MDX refined its API surface for better cross-framework compatibility and flexibility.
...
There's a `json-schema` plugin natively provided by xyzdocs MDX.
...
`createMDXSource` and `resolveFiles` are now relocated to `xyzdocs-mdx/runtime/next`.

This should not affect majority of projects, as `docs.toxyzdocsSource()` is now the preferred way.
...
- note that both docs and `create-xyzdocs-app` are updated to `.source` folder.
