...
xyzdocs v15 has no other significant changes other than migrating the config to Tailwind CSS v4.
...
Add `@import` to the presets exported by xyzdocs UI, and include the `xyzdocs-ui` package in the source.
...
@source '../node_modules/xyzdocs-ui/dist/**/*.js';
```
...
xyzdocs no longer use `--fd-<color>` like `--fd-primary: 0 0% 0%` for colors, it directly defines and uses colors in `@theme`.
...
v15 improves the playground UI (inspired by the minimalism of Scalar), and brought Scalar API Client support to xyzdocs OpenAPI.
...
<Callout>
  Be careful that you must configure Tailwind CSS first, using the pre-built stylesheet of xyzdocs
...
In the future, we also want to make further improvements to xyzdocs MDX:
