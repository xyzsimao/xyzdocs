---
title: xyzdocs MDX 14
description: Performance Improvements & API redesign.
author: Fuma Nama
date: 2025-11-16
---

We are pleased to announce xyzdocs MDX 14. This update introduces several major enhancements designed to:
...
xyzdocs MDX now generates multiple entry files in `.source` folder.
...
// [!code ++:20]
import browserCollections from 'xyzdocs-mdx:collections/browser';
...
For `doc` collections, `createMDXSource()` (Next.js) is renamed to `toxyzdocsSource()`.

If you use `create.sourceAsync()` (Vite) for `doc` collections, you should also switch to `toxyzdocsSource()`.
...
// or
import { postInstall } from 'xyzdocs-mdx/vite';
...
import { defineCollections, getDefaultMDXOptions } from 'xyzdocs-mdx/config';
...
import { defineCollections, applyMdxPreset } from 'xyzdocs-mdx/config';
import { myPlugin } from './remark-plugin';
...
import mdx from 'xyzdocs-mdx/vite';
...
Thank you for your continued support of xyzdocs. If you encounter any issues during the update, welcome to reach out through our GitHub Discussions.
