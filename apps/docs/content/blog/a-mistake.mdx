---
title: The early mistake of xyzdocs
...
The early mistake of xyzdocs was **xyzdocs CLI being too late and not getting enough mentions on docs**.
...
- Advanced customisations can use xyzdocs CLI, avoiding upstream changes.
...
Page tree is actually a crucial idea of xyzdocs, unifying all navigation & linking of docs.
...
xyzdocs is using CSS variables for all colors since the beginning, the naming is identical to Shadcn UI which makes customisation easy.
...
- **Continue improving xyzdocs CLI:** adding install-time optimization to remove unused files according to your options (e.g. use existing sidebar component instead of installing one).
- **A better versioning policy:** Reduce abstraction for mitigating upstream changes, instead version the components directly (e.g. creating a new `xyzdocs-ui/layouts/v2/page` instead of a major UI change).
- **Add xyzdocs-native cloud features:** More official integrations like monitoring/analytics feature designed for xyzdocs.
