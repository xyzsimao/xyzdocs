...
We are pleased to announce the release of xyzdocs 16.2, introducing a better layout system for xyzdocs UI.

## The New Layout System

Handling layouts in documentation frameworks can present significant challenges.

As a solution, xyzdocs UI implemented a [Grid System](/docs/ui/layouts/docs#the-layout-system) that is composable, flexible, cohesive and predictable.
...
For user-land customizations, we recommend the xyzdocs CLI customize command, or hooking the relevant grid areas like:
...
Previously, xyzdocs UI relied on `position: fixed` and mathematical computations (inspired by Vitepress). The older method often lacked composability and difficult for beginners to manage.
...
While xyzdocs 16.2 preserves compatibility in several areas, notably:
...
This decision enables xyzdocs UI to iterate these elements without introducing unexpected breaking changes to usage outside of API surfaces.

**Migration Steps:** Override the relevant layout components with your own implementations, or utilize the xyzdocs CLI commands (`add` or `customize`) to install them locally.
...
// For notebook layout
import { DocsPage } from 'xyzdocs-ui/layouts/notebook/page';
```

**Migration Steps:** Update your imports accordingly. Although the default `xyzdocs-ui/page` will redirect to the correct layout, we strongly advise explicit imports for clarity and future-proofing.
...
The `xyzdocs-ui/contexts/sidebar` module has been removed. Access the context via:
...
xyzdocs 16.2 aims to offer greater flexibility for developers. Welcome to share your thoughts about the new design on GitHub Discussion!
