...
Now you can generate types using `xyzdocs-mdx` command, it's recommended to set it as a `postinstall` script.
...
Now to integrate it with xyzdocs framework, change your `source.ts` from:

```ts
import { map } from '@/.map';
import { createMDXSource } from 'xyzdocs-mdx/runtime/next';
...
```ts
import { createMDXSource } from 'xyzdocs-mdx/runtime/next';
import type { InferMetaType, InferPageType } from 'xyzdocs-core/source';
import { loader } from 'xyzdocs-core/source';
...
```ts
import { defineDocs, defineCollections, frontmatterSchema } from 'xyzdocs-mdx/config';
import { z } from 'zod';
...
You can read the latest docs of xyzdocs MDX for details.

<Cards>
  <Card href="/docs/mdx" title="xyzdocs MDX">
