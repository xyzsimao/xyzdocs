...
xyzdocs use the `<Link />` component of your React framework, and keep their default prefetch behaviours.
...
Handling layout is challenging, xyzdocs UI needed an approach that is:
...
xyzdocs UI does this with a grid system:
