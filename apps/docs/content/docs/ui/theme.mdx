---
title: Themes
description: Add Theme to xyzdocs UI
icon: Palette
---
...
xyzdocs UI adds its own colors, animations, and utilities with Tailwind CSS preset.

### Setup

Only Tailwind CSS v4 is supported, the preset will also include source to xyzdocs UI itself:
...
For **Shadcn UI**, you can use the `shadcn` preset instead. xyzdocs UI will adopt colors from your Shadcn UI theme.

```css
@import 'tailwindcss';
@import 'xyzdocs-ui/css/shadcn.css';
@import 'xyzdocs-ui/css/preset.css';
```
...
xyzdocs supports light/dark modes with [`next-themes`](https://github.com/pacocoursey/next-themes), it is included in Root Provider.
...
```tsx
import { RootProvider } from 'xyzdocs-ui/provider/<framework>';
...
```css
@import 'xyzdocs-ui/css/<theme>.css';
@import 'xyzdocs-ui/css/preset.css';
```
...
> The plugin is supposed to work with xyzdocs UI's MDX components, it may conflict with `@tailwindcss/typography`.
>
