...
`loader()` provides an interface for xyzdocs to integrate with different content sources.
...
You can use it with content sources like xyzdocs MDX, see [`source`](/docs/headless/source-api/source) for details.
...
```tsx tab="Client"
import { usexyzdocsLoader } from 'xyzdocs-core/source/client';

function MyComponent() {
  // receives loader data
  const data = useLoaderData();
  const { pageTree } = usexyzdocsLoader(data);

  // now render it (e.g. via xyzdocs UI)
