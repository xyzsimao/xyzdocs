...
description: Use Content Collections for xyzdocs
---
...
To integrate with xyzdocs, add the following to your `content-collections.ts`.
...
import { transformMDX } from '@xyzdocs/content-collections/configuration';
...
To use components from other packages like xyzdocs UI, pass them to your `<MDXContent />` component.
...
<Callout title='Deep Dive: Why?'>
    ...
    To support importing a package from node modules, xyzdocs added a default value to the `cwd` option of MDX Bundler.
