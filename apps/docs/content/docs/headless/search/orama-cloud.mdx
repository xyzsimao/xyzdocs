...
2. Export the search indexes by pre-rendering a static route.
   ...
   import type { OramaDocument } from 'xyzdocs-core/search/orama-cloud';
...
3. Create a script to sync search indexes.
   ...
   import { sync, type OramaDocument } from 'xyzdocs-core/search/orama-cloud';
...
- Using [xyzdocs UI search dialog](/docs/search/orama-cloud).
- Custom search UI using the built-in hook of xyzdocs:
