...
description: Built-in document search of xyzdocs
---

xyzdocs supports document search with Orama, It is the default but also the recommended option since it can be self-hosted and totally free.
...
Usually, it is provided by your content source (e.g. xyzdocs MDX). You can also extract it from Markdown/MDX document using the [Remark Structure](/docs/headless/mdx/structure) plugin.
...
- **xyzdocs UI**: Supported out-of-the-box, see [Search UI](/docs/search/orama) for details.
- **Search Client**:
...
<auto-type-table type='Extract<import("xyzdocs-core/search/client").Client, { type: "fetch" }>' />
...
- **xyzdocs UI**: Configure [Tag Filter](/docs/search/orama#tag-filter) on Search UI.
...
- **xyzdocs UI**: use [static client](/docs/search/orama#static) on Search UI.
...
- **Search Client**: use `static` instead of `fetch`.
  ...
  <auto-type-table type='Extract<import("xyzdocs-core/search/client").Client, { type: "static" }>' />
...
import { source } from '@/lib/source';
import { createI18nSearchAPI } from 'xyzdocs-core/search/server';
...
- **xyzdocs UI**: No changes needed, xyzdocs UI handles this when you have i18n configured correctly.
- **Search Client**:
...
```ts
import { initAdvancedSearch } from 'xyzdocs-core/search/server';
