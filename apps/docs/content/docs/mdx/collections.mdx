...
By design, this will remove all default settings applied by your global config and xyzdocs MDX.
...
- #### `includeProcessedMarkdown`
  ...
  You can obtain the included content with `getText()` (e.g. in xyzdocs):
...
Define a collection for xyzdocs.
...
```ts
import { frontmatterSchema, metaSchema, defineDocs } from 'xyzdocs-mdx/config';
import { z } from 'zod';
