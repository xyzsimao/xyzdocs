...
Thanks to the flexible design, it is simple to implement access control with xyzdocs.
...
For example, this setup with xyzdocs MDX allows only MDX files with frontmatter `permission: public` to be shown.
...
This approach offers the best flexibility, but it doesn't rely on xyzdocs itself for access control, the setup may take longer and be more complicated.
