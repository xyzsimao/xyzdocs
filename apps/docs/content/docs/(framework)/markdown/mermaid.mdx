...
xyzdocs doesn't have a built-in Mermaid wrapper provided, we recommend using `mermaid` directly.

## Setup

Install the required dependencies, `next-themes` is used with xyzdocs to manage the light/dark mode.
...
```tsx tab="xyzdocs MDX" title="source.config.ts"
