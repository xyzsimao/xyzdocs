...
xyzdocs provides many useful extensions to MDX, a markup language. Here is a brief introduction to the default MDX syntax of xyzdocs.

> MDX is not the only supported format of xyzdocs. In fact, you can use any renderer such as headless CMS.
>
...
xyzdocs support YAML-based frontmatter by default, `title` represents the page title (`h1`) in xyzdocs UI.
...
```ts tab="xyzdocs MDX" title="source.config.ts"
...
import { remarkCodeTab } from 'xyzdocs-core/mdx-plugins';
...
> This is only available on **xyzdocs MDX**.
