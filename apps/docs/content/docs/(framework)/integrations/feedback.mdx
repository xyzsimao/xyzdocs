...
You can integrate a simple feedback system with xyzdocs.

## Installation

Install it using **xyzdocs CLI**.
...
```tsx
import { DocsPage } from 'xyzdocs-ui/layout/docs/page';
...
```tsx title="source.config.ts (xyzdocs MDX)"
import { remarkFeedbackBlock } from 'xyzdocs-core/mdx-plugins/remark-feedback-block';
