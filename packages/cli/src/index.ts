...
program
  .name('xyzdocs')
  .description('CLI to setup docs, init a config')
...
program
  .command('customise')
  .alias('customize')
  .description('simple way to customise layouts with docs UI')
  .option('--dir <string>', 'the root url or directory to resolve registry')
  .action(async (options: { config?: string; dir?: string }) => {
    await customise(
      createClientFromDir(options.dir, await createOrLoadConfig(options.config))
    )
  })

const dirShortcuts: Record<string, string> = {
  ':preview': 'https://preview.xyzdocs.dev/registry',
...
function createClientFromDir(
  dir = 'https://xyzdocs.dev/registry',
  config: LoadedConfig
) {
