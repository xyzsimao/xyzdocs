...
export async function customise(client: RegistryClient) {
  intro(picocolors.bgBlack(picocolors.whiteBright('Customise xyzdocs UI')));
  ...
  if (result.target === 'docs') {
    ...
    if (result.mode === 'minimal') {
      targets.push('xyzdocs/ui/layouts/docs-min');
    } else {
      targets.push(
    ...
    const maps: [string, string][] =
      result.mode === 'full-notebook'
        ? [
            ['xyzdocs-ui/layouts/notebook', '@/components/layout/notebook'],
            ['xyzdocs-ui/layouts/notebook/page', '@/components/layout/notebook/page'],
          ]
        : [
            ['xyzdocs-ui/layouts/docs', '@/components/layout/docs'],
            ['xyzdocs-ui/layouts/docs/page', '@/components/layout/docs/page'],
          ];
  ...
  if (result.target === 'home') {
    ...
    printNext(['xyzdocs-ui/layouts/home', `@/components/layout/home`]);
  }
